<script lang="ts">
  ...
  // The feeling is carried by its label and the bar height — one hue, one series.
...
{#if entries.length > 0}
  <div>
    <h3 class="text-xs font-semibold text-faint tracking-widest uppercase mb-3">Recent check-ins</h3>

    <div class="space-y-2">
      {#each entries as entry}
        <div class="flex items-center gap-3 rounded-sm bg-white border border-rule p-3.5">
          <!-- Feeling label -->
          ...
          <div class="flex gap-0.5 flex-shrink-0">
            {#each [1, 2, 3, 4, 5] as level}
              <div
                class="w-1.5 rounded-full transition-all {level <= entry.feeling ? 'bg-ink' : 'bg-rule'}"
